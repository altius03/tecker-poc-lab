...
상용 운영이나 고객사 배포를 목표로 하지는 않지만, Docker, Docker Compose, AWS 같은 인프라는 PoC 실행, 데모, 검증 용도로 사용할 수 있습니다.

> 신규 구현 기준: Python 3.11+ · gRPC + mTLS · PostgreSQL · Redpanda/Kafka-compatible queue · React/Vite dashboard · PDF report
...
-> PostgreSQL event store + transactional outbox
...
-> response plan
-> dashboard + report + gRPC/mTLS telemetry pipeline
```
...
| 구분 | 이유 |
|------|------|
| Packet payload | 탐지에 필요한 metadata보다 민감 원문이 많고 저장/전송 부담이 큼 |
| 임의의 HTTPS payload / HTTPS body | 로그인 정보, 메시지, 결제 정보가 섞일 수 있어 무단 감청처럼 보일 수 있음 |
| Message/chat content | EDR 탐지 목적보다 개인 대화 원문 수집 위험이 큼 |
| Browser password | credential theft와 구분이 어렵고 방어적 PoC에 필요하지 않음 |
| Keystroke / clipboard | keylogging/clipboard scraping처럼 보이며 Codex 작업 범위를 불필요하게 위험하게 만듦 |
| Document body | 문서 원문 대신 file hash, size, extension, timestamp만으로도 탐지 demo가 가능함 |

이 프로젝트의 원칙은 `원문 내용은 버리고, 탐지 가능한 metadata만 남긴다`입니다.
...
| Pipeline Delivery | gRPC + mTLS telemetry bundle, customer/device/version metadata |
...
│   ├── pipeline.py         # telemetry bundle / transport adapter
...
| 문서 | 내용 |
|------|------|
| `docs/rebuild-overall-blueprint.md` | 0부터 신규 구현하기 위한 전체 설계도 |
| `docs/implementation-decisions.md` | 신규 구현에서 확정한 수집/transport/backend/dashboard 결정 기록 |
| `docs/event-driven-queue.md` | PostgreSQL, Redpanda/Kafka-compatible queue, outbox, DLQ, event-driven 처리 설계 |
...
- 이전 정적 dashboard artifact에는 DB, login, real-time streaming server가 없습니다. 신규 구현은 PostgreSQL read model과 event-driven backend 기준입니다.
